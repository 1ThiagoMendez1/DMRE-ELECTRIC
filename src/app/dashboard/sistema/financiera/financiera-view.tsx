...
import { BillingModule } from "@/components/erp/billing-module"; // Ensure this handles its own data or accepts props

interface FinancieraViewProps {
    initialCuentas: CuentaBancaria[];
    initialMovimientos: MovimientoFinanciero[];
}

export function FinancieraView({ initialCuentas, initialMovimientos }: FinancieraViewProps) {
    ...
    // Use props as initial state or rely on Context actions for mutations
    // NOTE: In a perfect world, we'd use optimistic updates here. 
    // For now, we rely on the fact that Context updates might trigger re-renders, 
    // BUT we are detaching from Context data.
    // So we need local state initialized from props.

    // However, the actions (addCuentaBancaria) currently update the Context.
    // If we detach Context Data, those actions update a ghost state in Provider, not this View.
    // FIX: We need to update local state here OR keep using Context for *updates* but initial load from Server.

    // STRATEGY: Hybrid. 
    // 1. Initial render is fast (Server Data).
    // 2. We use React State here for the UI data.
    // 3. We wrap the Context Actions to update local state too. (Optimistic-ish)

    const [cuentas, setCuentas] = useState<CuentaBancaria[]>(initialCuentas);
    const [movimientosRaw, setMovimientosRaw] = useState<MovimientoFinanciero[]>(initialMovimientos);

    // Map movements
    const movimientos = movimientosRaw.map(m => ({
        ...m,
        valor: m.valor || (m as any).monto || 0,
        concepto: m.concepto || (m as any).descripcion || "Sin concepto",
        cuenta: m.cuenta || cuentas.find(c => c.id === m.cuentaId) || { nombre: "Cuenta Desconocida" }
    })) as any[];

    // Keep obligations as mock for now
    ...
    const handleCreateAccount = (newAccount: CuentaBancaria) => {
        addCuentaBancaria(newAccount); // Call server action wrapper
        setCuentas(prev => [newAccount, ...prev]); // Update local
    ...
    const handleUpdateAccount = (updatedAccount: CuentaBancaria) => {
        ...
        setCuentas(prev => prev.map(c => c.id === updatedAccount.id ? updatedAccount : c));
    ...
    const handleCreateTransaction = (newTransaction: MovimientoFinanciero) => {
        ...
        setMovimientosRaw(prev => [newTransaction, ...prev]);
    ...
    const handleUpdateTransaction = (updatedMov: MovimientoFinanciero) => {
        // updateMovimientoFinanciero(updatedMov); 
